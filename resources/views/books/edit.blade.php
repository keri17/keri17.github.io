@extends('layouts.app')
@section('title', 'Edit Book — BiblioVerse')

@section('content')

<h1 class="text-2xl font-bold mb-8">Edit Book</h1>

<form action="{{ route('admin.books.update', $book) }}" method="POST" class="max-w-2xl space-y-5">
    @csrf
    @method('PUT')
    ...
    <div>
        ...
        <select name="authors[]" multiple required size="5"
            ...
            @foreach($authors as $author)
                <option value="{{ $author->id }}" {{ $book->authors->pluck('id')->contains($author->id) ? 'selected' : '' }}>
                    {{ $author->name }}
                </option>
            @endforeach
        </select>
    ...
    <div class="flex items-center gap-3">
        <input type="checkbox" name="is_premium" value="1" id="is_premium" {{ old('is_premium', $book->is_premium) ? 'checked' : '' }}
    ...
    <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('books.show', $book) }}" class="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-slate-300 hover:bg-white/10 transition">
            Cancel
        </a>
    </div>
</form>
